...
namespace Проект
{
    public partial class ListMedicine : Form
    {
        public ListMedicine()
        {
            InitializeComponent();
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DeleteBool == true)
                {
                    var sqlCmd = new SqlCommand("DeleteMedicine", SC);
                    sqlCmd.CommandType = CommandType.StoredProcedure;
                    sqlCmd.Parameters.AddWithValue("@ID_Лекарства", Convert.ToInt32(dtUniversal[0, indeks].Value));
            ...
            {
                FormError f = new FormError();
                FormError.TextError = "Лекарство не может быть удалено," + '\n' + "так как уже используется !";
                f.ShowDialog();
            }
        }
