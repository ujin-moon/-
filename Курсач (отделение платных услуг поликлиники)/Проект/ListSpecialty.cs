...
namespace Проект
{
    public partial class ListSpecialty : Form
    {
        ...
        private void UpdateTable()
        {
            ...
            SqlCommand myCmd = new SqlCommand("PrintSpecialty1", SC);
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DeleteBool == true)
                {
                    var sqlCmd = new SqlCommand("DeleteSpecialty", SC);
                    sqlCmd.CommandType = CommandType.StoredProcedure;
                    sqlCmd.Parameters.AddWithValue("@ID_Специальности", Convert.ToInt32(dtUniversal[0, indeks].Value));
            ...
            {
                FormError f = new FormError();
                FormError.TextError = "Специальность не может быть удалена," + '\n' + "так как уже используется !";
                f.ShowDialog();
            }
        }
        ...
        {
            DataTable dt = new DataTable();
            SqlCommand myCmd = new SqlCommand("PoiskSpecialty1", SC);
            myCmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter da = new SqlDataAdapter(myCmd);
