...
namespace Проект
{
    public partial class ListChartView : Form
    {
        public ListChartView()
        {
            InitializeComponent();
        }
        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (DeleteBool == true)
                {
                    var sqlCmd = new SqlCommand("DeleteChartView", SC);
                    sqlCmd.CommandType = CommandType.StoredProcedure;
                    sqlCmd.Parameters.AddWithValue("@ID_ВидаГрафика", Convert.ToInt32(dtUniversal[0, indeks].Value));
            ...
            catch (System.ArgumentOutOfRangeException)
            {
                ...
                FormError.TextError = "Таблица видов графика работы пуста !";
                f.ShowDialog();
            }
            catch (System.Data.SqlClient.SqlException)
            {
                FormError f = new FormError();
                FormError.TextError = "Вид графика не может быть удален," + '\n' + "так как уже используется !";
                f.ShowDialog();
            }
        }
        ...
        private void PoiskTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            myCmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter da = new SqlDataAdapter(myCmd);
            da.SelectCommand.Parameters.Add(new SqlParameter("@Поиск", SqlDbType.VarChar, 5));
